...
export function validatePortfolio(portfolio) {
    ...
    if (
        portfolio.id &&
        !(ensureType(portfolio.id, "number") || ensureType(portfolio.id, "string"))
    ) {
    ...
    if (portfolio.secoes) {
        ...
        if (!Array.isArray(portfolio.secoes)) return null;
        ...
        if (portfolio.secoes.length) {
            for (let index = 0; index < portfolio.secoes.length; index++) {
                ...
                if (secao.contents) {
                    if (!Array.isArray(secao.contents)) return null;
                    ...
                    if (secao.contents.length) {
                        for (let index = 0; index < secao.contents.length; index++) {
                            ...
                            if (!ensureType(content.blob, "string")) {
                                return returnError(
                                    "validatePortfolio: content.blob não é valido(a)"
                                );
                            }

                            if (!ensureType(content.descricao, "string")) {
                                return returnError(
                                    "validatePortfolio: content.descricao não é valido(a)"
                                );
                            }
                        }
                    }
...
export function readPortfolios(...portfolios_id) {
    ...
    if (portfolios_id.length === 0) return portfolios;
...
export function updatePortfolio(portfolio_id, portfolio_new) {
    ...
    // 2. ensure number > 0
    if (!ensureType(portfolio_id, "number")) return null;

    if (portfolio_id < 1) return null;

    // Verifica se as informações do portfolio são validas
    if (!validatePortfolio(portfolio_new)) return null;
...
export function createPortfolio(portfolio_new) {
    ...
    if (!ensureType(portfolio_new.id, "number")) return null;
