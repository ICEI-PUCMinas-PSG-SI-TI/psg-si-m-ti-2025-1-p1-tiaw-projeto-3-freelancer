...
export function validateAvaliacao(avaliacao) {
    ...
    if (
        avaliacao.id &&
        !(ensureType(avaliacao.id, "number") || ensureType(avaliacao.id, "string"))
    ) {
...
export function readAvaliacoes(...avaliacoes_id) {
    ...
    if (avaliacoes_id.length === 0) return avaliacoes;
...
export function updateAvaliacao(avaliacao_id, avaliacoes_new) {
    ...
    // 2. ensure number > 0
    if (!ensureType(avaliacao_id, "number")) return null;

    if (avaliacao_id < 1) return null;

    // Verifica se as informações da avaliação são validas
    if (!validateAvaliacao(avaliacoes_new)) return null;
...
export function createAvaliacao(avaliacao_new) {
    ...
    if (!ensureType(avaliacao_new.id, "number")) return null;
